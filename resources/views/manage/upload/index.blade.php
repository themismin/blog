...
@section('content')
    <div class="row">
        <div class="col-xs-12">
            @include('manage.alert.success')
            @include('manage.alert.warning')
            <div class="box box-solid">
                <!-- /.box-header -->
                <div class="box-header">
                    <p>
                        <a href="{{ url('manage/file-upload') }}?dir={{ $dir }}" class="btn btn-info btn-sm">上传文件</a>
                                ...
                                @foreach($fileList['dirList'] as $dirName)
                                    <tr>
                                        <td>
                                            ...
                                            <a href="{{ route('manage.upload.index') }}?dir={{ $dirPath }}">
                                                {{ $dirName }}
                                            </a>
                                        </td>
                                        <td>dir</td>
                                        ...
                                        <td></td>
                                        <td>
                                            <a data-href='{{ route('manage.upload.dir-del') }}?dir={{ $dirPath }}'
                            ...
                            @if ($fileList['fileList'])
                                @foreach($fileList['fileList'] as $file)
                                    <tr>
                                        ...
                                        <td>{{ $file['path'] }}</td>
                                        <td>
                                            <a data-href='{{ route('manage.upload.file-del') }}?file={{ $dir.$file['file_name'] }}'
        ...
        function mkdir() {
            layer.open({
                ...
                yes: function(index, layero){
                    ...
                    Mingo.ajax.postForm("{{ route('manage.upload.mkdir') }}", {dir_name : dirName, dir:dir}, 'POST');
                }
            })
        }
    </script>
